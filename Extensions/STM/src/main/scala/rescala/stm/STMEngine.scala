...
object STMEngine {
  implicit val stm: Engine[STMTurn, STMTurn] = new TwoVersionEngineImpl[STMTurn, STMTurn]("STM", new STMTurn()) {
    override protected[rescala] def executeTurn[R](initialWrites: Traversable[Reactive], admissionPhase: STMTurn => R): R = atomic { tx => super.executeTurn(initialWrites, admissionPhase) }
  }
}
