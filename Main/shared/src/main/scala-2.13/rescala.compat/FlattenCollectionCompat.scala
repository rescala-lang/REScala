...
trait FlattenCollectionCompat {
  ...
  (implicit ticket: CreationTicket[S]): Flatten[Signal[T[Sig[B, S]], S], Signal[T[B], S]] = new Flatten[Signal[T[Sig[B, S]], S], Signal[T[B], S]] {
  ...
  [S <: Struct, B, T[U] <: IterableOps[U, T, T[U]], Evnt[A1, S1 <: Struct] <: Event[A1, S1]]
  (implicit ticket: CreationTicket[S]): Flatten[Signal[T[Evnt[B, S]], S], Event[B, S]] = new Flatten[Signal[T[Evnt[B, S]], S], Event[B, S]] {
  ...
  [S <: Struct, B, T[U] <: IterableOps[U, T, T[U]], Evnt[A1, S1 <: Struct] <: Event[A1, S1]]
  (implicit ticket: CreationTicket[S]): Flatten[Signal[T[Evnt[B, S]], S], Event[T[Option[B]], S]] = new Flatten[Signal[T[Evnt[B, S]], S], Event[T[Option[B]], S]] {
