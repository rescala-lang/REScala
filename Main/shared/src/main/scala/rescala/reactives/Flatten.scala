...
object Flatten {
  implicit def flattenSignal[A, S <: Struct, B](implicit ev: A <:< Signal[B, S]): Flatten[A, S, Signal[B, S]] = new Flatten[A, S, Signal[B, S]] {
    def apply(sig: Signal[A, S])(implicit ticket: Ticket[S]): Signal[B, S] = Signals.dynamic(sig) { s => s.depend(ev(s.depend(sig))) }
  }
  ...
  (implicit ev: A <:< T[Sig[B, S]], cbf: CanBuildFrom[T[_], B, T[B]]): Flatten[A, S, Signal[T[B], S]] = new Flatten[A, S, Signal[T[B], S]] {
    def apply(sig: Signal[A, S])(implicit ticket: Ticket[S]): Signal[T[B], S] = Signals.dynamic(sig) { s => ev(s.depend(sig)) map {(r: Signal[B, S]) => s.depend(r)} }
  }
  ...
  (implicit ev: A <:< Array[Sig[B, S]]): Flatten[A, S, Signal[Array[B], S]] = new Flatten[A, S, Signal[Array[B], S]] {
    def apply(sig: Signal[A, S])(implicit ticket: Ticket[S]): Signal[Array[B], S] = Signals.dynamic(sig) { s => ev(s.depend(sig)) map {(r: Signal[B, S]) => s.depend(r)} }
  }
  ...
  (implicit ev: A <:< Option[Sig[B, S]]): Flatten[A, S, Signal[Option[B], S]] = new Flatten[A, S, Signal[Option[B], S]] {
    def apply(sig: Signal[A, S])(implicit ticket: Ticket[S]): Signal[Option[B], S] = Signals.dynamic(sig) { s => ev(s.depend(sig)) map {(r: Signal[B, S]) => s.depend(r)} }
  }
  implicit def flattenEvent[A, S <: Struct, B](implicit ev: A <:< Event[B, S]): Flatten[A, S, Event[B, S]] = new Flatten[A, S, Event[B, S]] {
    def apply(sig: Signal[A, S])(implicit ticket: Ticket[S]): Event[B, S] = Events.dynamic(sig) { s => s.depend(s.depend(sig)) }
