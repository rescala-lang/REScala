...
object Flatten {
  ...
  [S <: Struct, B, Sig[U, V <: Struct] <: Signal[U, V]]
    ...
    def apply(sig: Signal[Sig[B, S], S]): Signal[B, S] = {Signals.dynamic(sig) { s => s.depend(s.depend(sig)) }}
  }
  ...
  [A, S <: Struct, B, Evnt[A1, S1 <: Struct] <: Event[A1, S1]]
    ...
    def apply(sig: Signal[Evnt[B, S], S]): Event[B, S] = Events.dynamic(sig) { s => s.depend(s.depend(sig)) }
  }
