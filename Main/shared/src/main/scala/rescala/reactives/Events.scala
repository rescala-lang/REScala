...
object Events {
  ...
  private abstract class ChangeEvent[T, S <: Struct](_bud: S#State[Pulse[Diff[T]], S], signal: Signal[T, S], name: REName)
    ...
    override protected[rescala] def reevaluate(turn: Turn[S], before: Pulse[Diff[T]], indeps: Set[InDep[S]]): ReevaluationResult[S] = {
      val pulse = {
  ...
  def static[T, S <: Struct](name: String, dependencies: InDep[S]*)(calculate: StaticTicket[S] => Pulse[T])(implicit maybe: CreationTicket[S]): Event[T, S] = maybe { initTurn =>
  ...
  def dynamic[T, S <: Struct](dependencies: InDep[S]*)(expr: DynamicTicket[S] => Option[T])(implicit maybe: CreationTicket[S]): Event[T, S] = {
    maybe { initialTurn =>
