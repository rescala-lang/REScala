...
trait ReevaluationStateAccess[S <: Struct] {
  type ReevOutResult
  private[rescala] def drop(node: Reactive[S], removeOutgoing: Reactive[S]): Unit
  private[rescala] def discover(node: Reactive[S], addOutgoing: Reactive[S]): Unit
  ...
  private[rescala] def writeIndeps(node: Reactive[S], indepsAfter: Set[Reactive[S]]): Unit
