...
trait Turn[S <: Struct] extends ReevaluationStateAccess[S] {
  private[rescala] def makeDynamicReevaluationTicket(indeps: Set[ReadableReactive[_, S]]): DynamicTicket[S]
...
trait TurnImpl[S <: Struct] extends Turn[S] with CreationImpl[S] with ComputationStateAccess[S] {
  private[rescala] def makeDynamicReevaluationTicket(indeps: Set[ReadableReactive[_, S]]): DynamicTicket[S] = new DynamicTicket[S](this, indeps)
