...
object TicketOrEngine extends LowPriorityTicketOrEngine {
  implicit def fromTicket[S <: Struct](implicit ticket: OutsidePropagationTicket[S]) = TicketOrEngine(Left(ticket))
}
sealed trait LowPriorityTicketOrEngine {
  implicit def fromEngine[S <: Struct](implicit engine: Engine[S, _ <: Turn[S]]) = TicketOrEngine(Right(engine))
}
