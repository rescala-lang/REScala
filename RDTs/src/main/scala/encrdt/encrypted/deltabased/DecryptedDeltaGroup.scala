...
case class DecryptedDeltaGroup[T](deltaGroup: T, dottedVersionVector: CausalContext) {
  def encrypt(aead: Aead)
             (implicit tJsonCodec: JsonValueCodec[T], dotSetJsonCodec: JsonValueCodec[CausalContext]): EncryptedDeltaGroup = {
...
object DecryptedDeltaGroup {
  implicit def decryptedDeltaGroupSemiLattice[T](implicit tLattice: SemiLattice[T]): SemiLattice[DecryptedDeltaGroup[T]] = (l, r) =>
    DecryptedDeltaGroup(SemiLattice[T].merged(l.deltaGroup, r.deltaGroup), l.dottedVersionVector.merged(r.dottedVersionVector))
}
