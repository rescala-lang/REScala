...
class MetaTest extends FunSuite {
  ...
  test("meta graph reification order test") {
    ...
    assert(
      fired == 1,
      "observers not triggered exactly once (as only one observer was added before the event was fired)"
    )
    fired = 0
    api.set(v, 3)
    api.fire(e, 1)
    ...
    assert(
      fired == 3,
      "observers not triggered exactly three times (twice through variable setting, once through event firing)"
    )
  }
