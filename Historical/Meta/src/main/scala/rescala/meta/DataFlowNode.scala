...
trait ReactiveNode[+T] extends DataFlowNode[T] {
  ...
  protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): ReactiV[Pulse[T], S]
...
trait EventNode[+T] extends ReactiveNode[T] {
  ...
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Event[T, S]
...
case class EvtEventNode[T](override var graph: DataFlowGraph) extends EventNode[T] {
  ...
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Evt[T, S] = {
...
case class ChangeEventNode[+T](override var graph: DataFlowGraph, base : SignalRef[T]) extends EventNode[Signals.Diff[T]] {
  ...
  override def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Event[Signals.Diff[T], S] = {
    registerReifier(reifier)
...
case class AndEventNode[T, U, +R](override var graph: DataFlowGraph, base : EventRef[T], other : EventRef[U], merger: (T, U) => R) extends EventNode[R] {
  ...
  override def dependencies: Set[DataFlowRef[_]] = Set(base, other)
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Event[R, S] = {
    registerReifier(reifier)
...
case class ZippedEventNode[+T, +U](override var graph: DataFlowGraph, base : EventRef[T], other : EventRef[U]) extends EventNode[(T, U)] {
  ...
  override def dependencies: Set[DataFlowRef[_]] = Set(base, other)
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Event[(T, U), S] = {
    registerReifier(reifier)
...
case class VarSignalNode[A](override var graph: DataFlowGraph) extends SignalNode[A] {
  ...
  override def dependencies: Set[DataFlowRef[_]] = Set()
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Var[A, S] = {
    registerReifier(reifier)
    reifier.createVar()
  }
...
case class MappedSignalNode[A, +U](override var graph: DataFlowGraph, base : SignalRef[A], mapping: (A) => U) extends SignalNode[U] {
  ...
  override def dependencies: Set[DataFlowRef[_]] = Set(base)
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S])(implicit ticket: CreationTicket[S]): Signal[U, S] = {
    registerReifier(reifier)
    base.deref.doReify(reifier).map(mapping)
  }
}
