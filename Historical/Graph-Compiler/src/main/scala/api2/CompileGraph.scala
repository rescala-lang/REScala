...
object CompileGraph {
  ...
  inline def withInput[IN <: Tuple: EventTupleUtils](inline appName: String)(
      inline dependencies: IN
  )(inline graph: CEventsFromEvents[IN] => Unit)(using TupleCodec[TupleFromEvents[IN]]): RemoteGraphWithInput[IN] = {
    macroCompiler.compileGraph(appName)(graph)

    given JsonValueCodec[OptionsFromEvents[IN]] =
      TupleCodecFactory.combineTupleCodecs(TupleCodecFactory.generateEventCodecsTuple[IN]).asInstanceOf[JsonValueCodec[
        OptionsFromEvents[IN]
      ]]
  ...
  inline def withOutput[OUT <: Tuple](inline appName: String)(inline graph: OUT)(using
      TupleUtils[TupleFromCEvents[OUT]],
      TupleCodec[TupleFromCEvents[OUT]]
  ): RemoteGraphWithOutput[TupleFromCEvents[OUT]] = {
    macroCompiler.compileGraph(appName)(graph)
  ...
  inline def withIO[OUT <: Tuple, IN <: Tuple: EventTupleUtils](inline appName: String)(
      inline dependencies: IN
  )(inline graph: CEventsFromEvents[IN] => OUT)(using
      TupleUtils[TupleFromCEvents[OUT]],
      TupleCodec[TupleFromCEvents[OUT]],
      TupleCodec[TupleFromEvents[IN]]
  ): RemoteGraphWithIO[IN, TupleFromCEvents[OUT]] = {
    macroCompiler.compileGraph(appName)(graph)
