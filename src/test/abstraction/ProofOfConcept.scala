...
class ProofOfConcept[T](set: Signal[Set[T]]) extends SignalWrapper {
	...
	implicit object Wrapping extends SignalWrappable1[Set, ProofOfConcept] {
	    def wrap[T](unwrapped: Signal[Set[T]]): ProofOfConcept[T] = new ProofOfConcept(unwrapped)
	}
	...
	val filter = (liftPure1(_.filter(_: T => Boolean)) _ andThen wrap1) 
	
	val filterSelf = liftMutating1(_.filter(_: T => Boolean)) _
	
	def map[B] = liftPure1(_.map(_: T => B)) _ andThen wrap1
