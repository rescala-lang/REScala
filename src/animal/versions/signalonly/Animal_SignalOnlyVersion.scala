...
abstract class Animal(override implicit val world: World) extends BoardElement {
  ...
  protected def randomMove: AnimalState = {
	...
	override def tick: Unit = {
	  super.tick()
	  ticks.set(ticks.now + 1)
	  energy.set(energy.now + energyGain.now - energyDrain.now)
	}
...
trait Female extends Animal {
  ...
  override def tick: Unit = {
      super.tick
      if(isPregnant.now){
        if(pregnancyTime.now > 0){
          pregnancyTime.set(pregnancyTime.now - 1)
          if(pregnancyTime.now == 0)
            giveBirth()
        }
      }
...
class Plant(override implicit val world: World) extends BoardElement {
  ...
  override def tick: Unit = {
    ...
    val oldSize = size.now
    age.set(age.now + 1)
    
    if(size.now != oldSize){
       if(size.now == Plant.MaxSize)
    	   germinate() // spawn a new plant in proximity to this one
	 }
  }
...
class World {
  ...
  def tick() = {
    val oldDay = time.day.now
    val oldWeek = time.week.now
    ...
    if(time.day.now != oldDay){
      dayChanged()
    }
      
    if(time.week.now != oldWeek){
      weekChanged()
    }
  }
