...
trait NotificationAction[N <: ReSource[FullMVStruct]] extends ReevaluationHandling[N] {
  ...
  def processNotificationResult(
      ...
      notificationResultAction: NotificationBranchResult[FullMVTurn, Derived[FullMVStruct]]
  ): Unit = {
...
class SourceNotification(
    ...
    override val node: ReSource[FullMVStruct],
    override val changed: Boolean
) extends NotificationAction[ReSource[FullMVStruct]]
    with SourceReevaluationHandling {
  override def deliverNotification(): (Boolean, NotificationBranchResult[FullMVTurn, Derived[FullMVStruct]]) =
...
class Notification(
    ...
    override val changed: Boolean
) extends NotificationAction[Derived[FullMVStruct]]
