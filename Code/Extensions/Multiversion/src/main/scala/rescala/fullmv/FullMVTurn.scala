...
trait FullMVTurn
    extends Initializer[FullMVStruct]
...
//  val completionWaiters = AtomicReference[List[() => Unit]]
  ...
  override def accessTicket(): AccessTicket[FullMVStruct] =
    new AccessTicket[FullMVStruct]() {
  ...
  override def makeDerivedStructState[V](valuePersistency: V)
      : NonblockingSkipListVersionHistory[V, FullMVTurn, ReSource[FullMVStruct], Derived[FullMVStruct]] = {
    val state = new NonblockingSkipListVersionHistory[V, FullMVTurn, ReSource[FullMVStruct], Derived[FullMVStruct]](
      host.dummy,
      valuePersistency
    )
  ...
  override def ignite(
      reactive: Derived[FullMVStruct],
      incoming: Set[ReSource[FullMVStruct]],
...
//    assert(Thread.currentThread() == userlandThread, s"$this ignition of $reactive on different thread ${Thread.currentThread().getName}")
    ...
    ignitionNotification.deliverNotification() match {
      ...
      case (false, DoNothing) =>
        if (FullMVEngine.DEBUG)
          println(s"[${Thread.currentThread().getName}] $this ignite $reactive did not spawn a branch or reevaluation.")
        activeBranchDifferential(TurnPhase.Executing, -1)
...
//    assert((successorWrittenVersions ++ maybeFollowFrame).forall(retrofit => retrofit == this || retrofit.isTransitivePredecessor(this)), s"$this retrofitting contains predecessors: drop $node -> $removeOutgoing retrofits $r from ${node.state}")
  ...
  private[rescala] def writeIndeps(node: Derived[FullMVStruct], indepsAfter: Set[ReSource[FullMVStruct]]): Unit =
