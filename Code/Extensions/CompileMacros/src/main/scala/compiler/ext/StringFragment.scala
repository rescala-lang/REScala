...
object StringFragment extends StringIFFragment with TermIFFragment with ApplyIFFragment with TypeIFFragment with SerializationIFFragment {
  ...
  def toStringFunName(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using TranslationContext): String =
  ...
  def toStringWithUnknownLength(using Quotes)(format: String, expr: CExpr, tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: FunctionDeclTC): CExpr =
    val funName = toStringFunName(tpe)
    val f = ctx.nameToFunctionDecl.getOrElseUpdate(funName, {
