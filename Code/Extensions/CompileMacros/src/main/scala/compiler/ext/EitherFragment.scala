...
object EitherFragment extends SelectIFFragment with ApplyIFFragment with MatchIFFragment with TypeIFFragment with DataStructureIFFragment with StringIFFragment with SerializationIFFragment {
...

      {
        case tpe if tpe <:< TypeRepr.of[Either[?, ?]] =>
          ...
          val refCountFieldDecl =
            if dispatch[DataStructureIFFragment](_.usesRefCount)(leftType) || dispatch[DataStructureIFFragment](_.usesRefCount)(rightType) then
              List(CFieldDecl(refCountField, CPointerType(CIntegerType)))
            else Nil

          CRecordDecl(
            "Either_" + dispatch[TypeIFFragment](_.typeName)(leftType) + "_" + dispatch[TypeIFFragment](_.typeName)(rightType),
  ...
  override def usesRefCount(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.TypeRepr, Boolean] = {
...

      {
        case tpe if tpe <:< TypeRepr.of[Either[?, ?]] =>
          val typeArgs(List(leftType, rightType)) = tpe.widen: @unchecked
          dispatch[DataStructureIFFragment](_.usesRefCount)(leftType) || dispatch[DataStructureIFFragment](_.usesRefCount)(rightType)
      }
    }
  ...
  override def serializationRetainsEquality(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.TypeRepr, Boolean] = {
...

      {
        case tpe if tpe <:< TypeRepr.of[Either[?, ?]] =>
          val typeArgs(List(leftType, rightType)) = tpe.widen: @unchecked
          dispatch[SerializationIFFragment](_.serializationRetainsEquality)(leftType) && dispatch[SerializationIFFragment](_.serializationRetainsEquality)(rightType)
      }
    }
  ...
  private def getLeftCreator(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
    ctx.recordFunMap.getOrElseUpdate(dispatch[TypeIFFragment](_.typeName)(tpe) -> CREATE_LEFT, {
  ...
  private def getRightCreator(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
    ctx.recordFunMap.getOrElseUpdate(dispatch[TypeIFFragment](_.typeName)(tpe) -> CREATE_RIGHT, {
  ...
  private def getEitherEquals(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getEitherDeepCopy(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getEitherPrinter(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getEitherToString(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getEitherSerialize(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getEitherDeserialize(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
