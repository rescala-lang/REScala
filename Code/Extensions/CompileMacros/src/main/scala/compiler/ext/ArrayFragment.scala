...
object ArrayFragment extends SelectIFFragment with ApplyIFFragment with MatchIFFragment with TypeIFFragment with DataStructureIFFragment with StringIFFragment with SerializationIFFragment {
  ...
  override def compileApply(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.Apply, CExpr] = ensureCtx[RecordDeclTC] {
...

      {
        ...
        case apply@Apply(TypeApply(Select(Ident("Array"), "ofDim"), List(tpt)), List(n)) if TypeFragment.hasDefaultValue(tpt.tpe) =>
          CCallExpr(
            getArrayFill(apply.tpe).ref,
            List(
              dispatch[TermIFFragment](_.compileTermToCExpr)(n),
    ...
    PartialFunction[quotes.reflect.TypeRepr, CRecordDecl] = {
...

      {
        case tpe if tpe <:< TypeRepr.of[Array[?]] =>
          ...
          CRecordDecl("Array_" + dispatch[TypeIFFragment](_.typeName)(elemType), List(dataFieldDecl, lengthFieldDecl, refCountFieldDecl))
      }
    }
  ...
  private def arrayStructInitializer(using Quotes)(tpe: quotes.reflect.TypeRepr, sizeExpr: CExpr)
                                    (using FragmentedCompiler)(using RecordDeclTC): CExpr = {
    import quotes.reflect.*
  ...
  private def getArrayCreator(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def arrayIndexAccess(using Quotes)(arr: quotes.reflect.Term, idx: quotes.reflect.Term)(using FragmentedCompiler)(using TranslationContext): CArraySubscriptExpr = {
    CArraySubscriptExpr(
      CMemberExpr(
        dispatch[TermIFFragment](_.compileTermToCExpr)(arr),
        dataField
  ...
  private def arrayIndexUpdate(using Quotes)(arr: quotes.reflect.Term, idx: quotes.reflect.Term, v: quotes.reflect.Term)
                              (using FragmentedCompiler)(using TranslationContext): CExpr = {
  ...
  private def getArrayFill(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
    ctx.recordFunMap.getOrElseUpdate(dispatch[TypeIFFragment](_.typeName)(tpe) -> FILL, {
      val recordDecl = getRecordDecl(tpe)
      val CFieldDecl(_, CQualType(CPointerType(elemCType), _)) = recordDecl.getField(dataField): @unchecked
  ...
  private def getArrayDeepCopy(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getArrayPrinter(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getArrayToString(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getArraySerialize(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
  ...
  private def getArrayDeserialize(using Quotes)(tpe: quotes.reflect.TypeRepr)(using FragmentedCompiler)(using ctx: RecordDeclTC): CFunctionDecl = {
