...
trait MatchIFFragment extends CompilerFragment {
  def compileMatchToCStmt(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.Match, CStmt] = PartialFunction.empty

  def compileMatchToCExpr(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.Match, CExpr] = PartialFunction.empty

  def compileCaseDef(using Quotes)(using FragmentedCompiler)(using TranslationContext):
  ...
  def compilePattern(using Quotes)(using FragmentedCompiler)(using TranslationContext):
