...
object DefinitionFragment extends DefinitionIFFragment {
  override def compileDefinition(using Quotes)(using FragmentedCompiler)(using TranslationContext):
  ...
  override def compileDefDef(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.DefDef, CFunctionDecl] = ensureCtx[FunctionDeclTC] { ctx ?=> defDef =>
      ...
      val plainBody: Option[CCompoundStmt] = rhs.map { (t: Term) =>
        t match
          ...
          case Return(expr, _) => CCompoundStmt(List(CReturnStmt(Some(dispatch[TermIFFragment](_.compileTermToCExpr)(expr)))))
          case term if term.tpe =:= TypeRepr.of[Unit] => CCompoundStmt(List(dispatch[TermIFFragment](_.compileTermToCStmt)(term)))
  ...
  override def compileValDefToCVarDecl(using Quotes)(using fc: FragmentedCompiler)(using TranslationContext):
  ...
  override def compileValDefToCParmVarDecl(using Quotes)(using FragmentedCompiler)(using TranslationContext):
