...
trait RefIFFragment extends CompilerFragment {
  def compileRef(using Quotes)(using FragmentedCompiler)(using TranslationContext):
    PartialFunction[quotes.reflect.Ref, CExpr] = PartialFunction.empty
  
  def compileIdent(using Quotes)(using FragmentedCompiler)(using TranslationContext):
