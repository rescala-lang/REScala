...
object MatchFragment extends MatchIFFragment {
  override def compileMatchToCStmt(using Quotes)(using FragmentedCompiler)(using ctx: TranslationContext):
    PartialFunction[quotes.reflect.Match, CStmt] = {
...

      {
        case Match(scrutinee, cases) =>
          ...
          val (lastCond, lastDecls, lastStmtList) = dispatch[MatchIFFragment](_.compileCaseDef)(cases.last, scrutineeDecl.ref, scrutinee.tpe)
          val lastIf = CIfStmt(
          ...
          val ifStmt = cases.init.foldRight(lastIf) {
            case (cd, nextIf) =>
              val (cond, decls, stmtList) = dispatch[MatchIFFragment](_.compileCaseDef)(cd, scrutineeDecl.ref, scrutinee.tpe)
  ...
  override def compileMatchToCExpr(using Quotes)(using FragmentedCompiler)(using ctx: TranslationContext):
        ...
        case (CaseDef(pattern, guard, rhs), scrutineeExpr, scrutineeType) =>
          ...
          guard match {
            ...
            case Some(guardExpr) =>
              ...
              val guardCompiled = replaceBoundIdentifiers.mapCExpr(dispatch[TermIFFragment](_.compileTermToCExpr)(guardExpr))
  ...
  override def compilePattern(using Quotes)(using FragmentedCompiler)(using ctx: TranslationContext):
