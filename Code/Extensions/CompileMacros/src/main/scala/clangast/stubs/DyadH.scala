...
object DyadH extends CLibraryStub {
  ...
  def DYAD_EVENT_NULL(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_NULL", None))
  def DYAD_EVENT_DESTROY(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_DESTROY", None))
  def DYAD_EVENT_ACCEPT(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_ACCEPT", None))
  def DYAD_EVENT_LISTEN(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_LISTEN", None))
  def DYAD_EVENT_CONNECT(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_CONNECT", None))
  def DYAD_EVENT_CLOSE(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_CLOSE", None))
  def DYAD_EVENT_READY(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_READY", None))
  def DYAD_EVENT_DATA(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_DATA", None))
  def DYAD_EVENT_LINE(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_LINE", None))
  def DYAD_EVENT_ERROR(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_ERROR", None))
  def DYAD_EVENT_TIMEOUT(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_TIMEOUT", None))
  def DYAD_EVENT_TICK(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_EVENT_TICK", None))

  def DYAD_STATE_CLOSED(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_STATE_CLOSED", None))
  def DYAD_STATE_CLOSING(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_STATE_CLOSING", None))
  def DYAD_STATE_CONNECTING(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_STATE_CONNECTING", None))
  def DYAD_STATE_CONNECTED(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_STATE_CONNECTED", None))
  def DYAD_STATE_LISTENING(using TranslationContext): CEnumConstantDecl = includeStub(CEnumConstantDecl("DYAD_STATE_LISTENING", None))
  ...
  def dyad_setUpdateTimeout(using TranslationContext): CFunctionDecl = includeStub(CFunctionStub("dyad_setUpdateTimeout"))
  ...
  def dyad_removeAllListeners(using TranslationContext): CFunctionDecl = includeStub(CFunctionStub("dyad_removeAllListeners"))
  ...
  def dyad_getBytesReceived(using TranslationContext): CFunctionDecl = includeStub(CFunctionStub("dyad_getBytesReceived"))
