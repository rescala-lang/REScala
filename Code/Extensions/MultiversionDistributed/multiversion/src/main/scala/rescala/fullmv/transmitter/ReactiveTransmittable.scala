...
object ReactiveTransmittable {
      ...
      this match {
        ...
        case TurnTrySubsume(turn, lockedNewParent) =>
          allEmpty("TurnTrySubsume").copy(_2 =
            (turn, TurnPhase.Uninitialized, None, None) :: (lockedNewParent, TurnPhase.Uninitialized, None, None) :: Nil
          )
        ...
        case LockTrySubsume(lock, lockedNewParent) =>
          allEmpty("LockTrySubsume").copy(_2 =
            (lock, TurnPhase.Uninitialized, None, None) :: (lockedNewParent, TurnPhase.Uninitialized, None, None) :: Nil
          )
  ...
  object Message {
    def fromTuple[P](tuple: Msg[P]): Message[P] =
      tuple match {
        ...
        case ("Initialize", (maybeFirstFrame, maybeFirstPhase, maybeFirstFrameTree, _) :: initValues, _) => Initialize(
            initValues.map {
        ...
        case (
              ...
              (turn, phase, Some((tree, clock)), Some(value)) :: (fTurn, fPhase, Some((fPredTree, fPredClock)), _) :: _,
              _
        ...
        case ("MaybeNewReachableSubtree", (turn, _, _, _) :: (attachBelow, attachPhase, Some((tree, _)), _) :: _, _) =>
  ...
  implicit def unitResponseToUnitFuture(future: Future[UnitResponse.type]): Future[Unit] =
    future.map(_ => ())(FullMVEngine.notWorthToMoveToTaskpool)
  implicit def unitToUnitResponseFuture(future: Future[Unit]): Future[UnitResponse.type] =
    future.map(_ => UnitResponse)(FullMVEngine.notWorthToMoveToTaskpool)
  ...
  case class AddPredecessor(turn: Host.GUID, predecessorTree: CaseClassTransactionSpanningTreeNode[TurnPushBundle])
      extends UnderlyingChatterRequest { override type Response = BooleanResponse }
  ...
  ) extends UnderlyingChatterRequest { override type Response = UnitResponse.type }
  ...
  implicit def signalTransmittable[P, S](implicit
    ...
    new ReactiveTransmittable[P, Signal[P, FullMVStruct], S] {
      override def instantiate(
          state: FullMVState[Pulse[P], FullMVTurn, ReSource[FullMVStruct], Derived[FullMVStruct]],
          initTurn: FullMVTurn,
          name: String
      ): ReactiveReflectionImpl[Pulse[P]] with Signal[P, FullMVStruct] =
        new ReactiveReflectionImpl[Pulse[P]](host, None, state, s"SignalReflection($name)")
          with Signal[P, FullMVStruct] {
  ...
  implicit def eventTransmittable[P, S](implicit
      ...
      serializable: Serializable[S]
  ): Transmittable[Event[P, FullMVStruct], S, Event[P, FullMVStruct]] =
    new ReactiveTransmittable[P, Event[P, FullMVStruct], S] {
      override def instantiate(
          state: FullMVState[Pulse[P], FullMVTurn, ReSource[FullMVStruct], Derived[FullMVStruct]],
          initTurn: FullMVTurn,
          name: String
      ): ReactiveReflectionImpl[Pulse[P]] with Event[P, FullMVStruct] =
