...
object LastWriterWins {
  implicit def LWWRegLattice[O, T](using Ordering[O]): Lattice[LastWriterWins[O, T]] =
    (left, right) => Ordering[O].compare(left.timestamp, right.timestamp) match
      case 0 => if (left == right) then left else throw IllegalStateException(s"LWW same timestamp, different value: »$left«, »$right«")
