...
object Flatten extends rescala.compat.FlattenCollectionCompat {
  ...
  implicit def arraySignals[S <: Struct, B: ClassTag, Sig[U] <: Signal[U, S]](implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ): Flatten[Signal[Array[Sig[B]], S], Signal[Array[B], S]] =
    new Flatten[Signal[Array[Sig[B]], S], Signal[Array[B], S]] {
      def apply(sig: Signal[Array[Sig[B]], S]): Signal[Array[B], S] =
        api.Signals.dynamic(sig) { t => t.depend(sig) map { r: Signal[B, S] => t.depend(r) } }
    }
  ...
  implicit def optionSignal[S <: Struct, B, Sig[U] <: Signal[U, S]](implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ): Flatten[Signal[Option[Sig[B]], S], Signal[Option[B], S]] =
    new Flatten[Signal[Option[Sig[B]], S], Signal[Option[B], S]] {
      def apply(sig: Signal[Option[Sig[B]], S]): Signal[Option[B], S] =
        api.Signals.dynamic(sig) { t => t.depend(sig) map { r: Signal[B, S] => t.depend(r) } }
    }

  /** Flatten a Signal[Event[B]]\] into a Event[B] where the new Event fires whenever the current inner event fires */
  implicit def event[A, S <: Struct, B, Evnt[A1, S1 <: Struct] <: Event[A1, S1]](implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ): Flatten[Signal[Evnt[B, S], S], Event[B, S]] =
    new Flatten[Signal[Evnt[B, S], S], Event[B, S]] {
  ...
  implicit def option[A, S <: Struct, B](implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ): Flatten[Event[Option[B], S], Event[B, S]] =
    new Flatten[Event[Option[B], S], Event[B, S]] {
      def apply(event: Event[Option[B], S]): Event[B, S] =
        api.Events.static(event) { t => t.dependStatic(event).flatten }
    }
