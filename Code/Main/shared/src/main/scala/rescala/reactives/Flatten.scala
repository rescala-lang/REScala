...
object Flatten extends rescala.compat.FlattenCollectionCompat {
  ...
  : Flatten[Signal[Sig[B, S], S], Signal[B, S]] = new Flatten[Signal[Sig[B, S], S], Signal[B, S]] {
    def apply(sig: Signal[Sig[B, S], S]): Signal[B, S] = api.Signals.dynamic(sig) { t => t.depend(t.depend(sig)) }
  }
  ...
  [S <: Struct, B: ClassTag, Sig[U, V <: Struct] <: Signal[U, V]](implicit ticket: CreationTicket[S], api: RescalaInterface[S])
  : Flatten[Signal[Array[Sig[B, S]], S], Signal[Array[B], S]] = new Flatten[Signal[Array[Sig[B, S]], S], Signal[Array[B], S]] {
    def apply(sig: Signal[Array[Sig[B, S]], S]): Signal[Array[B], S] = api.Signals.dynamic(sig) { t => t.depend(sig) map { r: Signal[B, S] => t.depend(r) } }
  }
  ...
  [S <: Struct, B, Sig[U, V <: Struct] <: Signal[U, V]](implicit ticket: CreationTicket[S], api: RescalaInterface[S])
  : Flatten[Signal[Option[Sig[B, S]], S], Signal[Option[B], S]] = new Flatten[Signal[Option[Sig[B, S]], S], Signal[Option[B], S]] {
    def apply(sig: Signal[Option[Sig[B, S]], S]): Signal[Option[B], S] = api.Signals.dynamic(sig) { t => t.depend(sig) map { r: Signal[B, S] => t.depend(r) } }
  }
