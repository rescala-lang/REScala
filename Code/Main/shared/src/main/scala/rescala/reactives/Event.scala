...
trait Event[+T, S <: Struct] extends ReSource[S] with InterpMacro[Option[T], S] with Disconnectable[S] {
  ...
  final def collect[U](expression: PartialFunction[T, U])(implicit ticket: CreationTicket[S]): Event[U, S] =
    macro rescala.macros.ReactiveMacros.ReactiveUsingFunctionMacro[
      ...
      U,
      S,
      Events.CollectFuncImpl.type,
      rescalaAPI.Events.type
    ]
  ...
  final def dropParam(implicit ticket: CreationTicket[S]): Event[Unit, S] =
    rescalaAPI.Events.static(this)(_ => Some(()))
  ...
  /** Folds events with a given operation to create a Signal.
    ...
    */
  @cutOutOfUserComputation
  final def fold[A](init: A)(op: (A, T) => A)(implicit ticket: CreationTicket[S]): Signal[A] =
  ...
  /** Switch back and forth between two signals on occurrence of event e
    ...
    */
  @cutOutOfUserComputation
  final def toggle[A](a: Signal[A], b: Signal[A])(implicit ticket: CreationTicket[S]): Signal[A] =
