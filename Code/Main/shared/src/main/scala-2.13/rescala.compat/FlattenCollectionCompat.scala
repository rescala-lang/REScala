...
trait FlattenCollectionCompat {
  ...
  implicit def traversableSignals[S <: Struct, B, T[U] <: IterableOps[U, T, T[U]], Sig[A1] <: Signal[A1, S]](implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ): Flatten[Signal[T[Sig[B]], S], Signal[T[B], S]] =
    new Flatten[Signal[T[Sig[B]], S], Signal[T[B], S]] {
      def apply(sig: Signal[T[Sig[B]], S]): Signal[T[B], S] =
  ...
  /** Flatten a Signal[Traversable[Event[B]\]\] into a Event[B]. The new Event fires the value of any inner firing Event.
    ...
    */
  def firstFiringEvent[S <: Struct, B, T[U] <: IterableOps[U, T, T[U]], Evnt[A1, S1 <: Struct] <: Event[A1, S1]](
      implicit
      ticket: CreationTicket[S],
      api: RescalaInterface[S]
  ...
  def traversableOfAllOccuringEventValues[S <: Struct, B, T[U] <: IterableOps[U, T, T[U]], Evnt[
      A1,
      S1 <: Struct
