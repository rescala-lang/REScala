...
trait FlattenCollectionCompat {
  ...
  implicit def traversableSignals[S <: Struct, B, T[U] <: TraversableLike[U, T[U]], Sig[A1, S1 <: Struct] <: Signal[
    A1,
    S1
  ]](implicit
  ...
  ): Flatten[Signal[T[Sig[B, S]], S], Signal[T[B], S]] =
    new Flatten[Signal[T[Sig[B, S]], S], Signal[T[B], S]] {
      def apply(sig: Signal[T[Sig[B, S]], S]): Signal[T[B], S] =
  ...
  /** Flatten a Signal[Traversable[Event[B]\]\] into a Event[B]. The new Event fires the value of any inner firing Event.
    ...
    */
  def firstFiringEvent[S <: Struct, B, T[U] <: TraversableLike[U, T[U]], Evnt[A1, S1 <: Struct] <: Event[A1, S1]](
      implicit
      ticket: CreationTicket[S],
      cbf: CanBuildFrom[T[_], Option[B], T[Option[B]]],
      api: RescalaInterface[S]
  ): Flatten[Signal[T[Evnt[B, S]], S], Event[B, S]] =
    new Flatten[Signal[T[Evnt[B, S]], S], Event[B, S]] {
      def apply(sig: Signal[T[Evnt[B, S]], S]): Event[B, S] =
        api.Events.dynamic(sig) { t =>
  ...
  def traversableOfAllOccuringEventValues[S <: Struct, B, T[U] <: TraversableLike[U, T[U]], Evnt[
      A1,
      S1 <: Struct
  ] <: Event[A1, S1]](implicit
      ticket: CreationTicket[S],
      cbf: CanBuildFrom[T[_], Option[B], T[Option[B]]],
      api: RescalaInterface[S]
  ): Flatten[Signal[T[Evnt[B, S]], S], Event[T[Option[B]], S]] =
    new Flatten[Signal[T[Evnt[B, S]], S], Event[T[Option[B]], S]] {
      def apply(sig: Signal[T[Evnt[B, S]], S]): Event[T[Option[B]], S] =
        api.Events.dynamic(sig) { t =>
          val all = t.depend(sig) map { r: Event[B, S] => t.depend(r) }
