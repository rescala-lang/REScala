...
import rescala.extra.lattices.delta.crdt.{AWSet, AWSetCRDT, ORMap}
import rescala.extra.lattices.delta.crdt.ORMap._
...
class ORMapTest extends AnyFreeSpec with ScalaCheckDrivenPropertyChecks {
  ...
  "mutateKey/queryKey" in { (add: List[Int], remove: List[Int], k: Int) =>
    ...
    val map = {
      val added = add.foldLeft(ORMap[Int, AWSet.Embedded[Int], DietMapCContext](aea)) {
        case (m, e) => m.mutateKey(k, AWSetCRDT.add(e))
      }

      remove.foldLeft(added) {
        case (m, e) => m.mutateKey(k, AWSetCRDT.remove(e))
      }
    }

    val mapElements = map.queryKey(k, AWSetCRDT.elements)
  ...
  "remove" in { (add: List[Int], remove: List[Int], k: Int) =>
    ...
    val queryResult = removed.queryKey(k, AWSetCRDT.elements)
