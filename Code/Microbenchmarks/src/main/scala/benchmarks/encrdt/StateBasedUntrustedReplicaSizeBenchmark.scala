  ...
  for (totalElements <- (minElementExponent to maxElementExponent).map(i => math.pow(10, i.toDouble).toInt)) {
    ...
    for (parallelUpdates <- 1 to MAX_PARALLEL_UPDATES) {
      ...
      for (replicaId <- 1 to parallelUpdates) {
        ...
        decryptedStatesMerged = DecryptedState.lattice[StateType[String, String]](DeltaAddWinsLastWriterWinsMap.deltaAddWinsMapLattice).merge(decryptedStatesMerged, replicaSpecificDecState)
      }
