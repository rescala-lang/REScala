...
object ObjectSizes {
  ...
  def main(args: Array[String]): Unit = {
    ...
    measure(
      "reev ticket",
      new rescala.default.ReevTicket(ptx, ()) {
    ...
    measure(
      "nolock reev ticket",
      new rescala.Schedulers.synchron.ReevTicket(stx, ()) {
        override protected def staticAccess(reactive: rescala.Schedulers.synchron.ReSource): reactive.Value  = ???
        override protected def dynamicAccess(reactive: rescala.Schedulers.synchron.ReSource): reactive.Value = ???
      }
      )
  }
