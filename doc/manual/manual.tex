...
println(a.getValue,b.getValue,c.getValue) // -> (2,3,5)
a()= 4  (*@\label{updated}@*)
println(a.getValue,b.getValue,c.getValue) // -> (4,3,7)
\end{codenv}
...
signal or a var can be accessed using the \code{getValue} method. For
example:
...
val x: Int = a.getValue
val y: Int = s.getValue
val z: Boolean = t.getValue
println(z)
\end{codenv}
...
while (true) {
  Thread sleep 20
  time() = time.getValue + 1  (*@\label{increasetime}@*)
}
...
Note that using \code{println(space.getValue)} would also print the
...
\code{getValue} operator returns the current value but does {\it not}
...
which this behavior is desirable, using \code{getValue} inside a signal
