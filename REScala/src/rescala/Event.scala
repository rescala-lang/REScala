...
trait Event[+T] extends Pulsing[T] {
   ...
   * Events disjunction.
   */
  final def ||[U >: T](other: Event[U])(implicit maybe: Ticket): Event[U] = Events.or(this, other)

  /**
   * Event filtered with a predicate
   */
  final def &&(pred: T => Boolean)(implicit maybe: Ticket): Event[T] = Events.filter(this)(pred)
  final def filter(pred: T => Boolean)(implicit maybe: Ticket): Event[T] = &&(pred)

  /**
   * Event is triggered except if the other one is triggered
   */
  final def \[U](other: Event[U])(implicit maybe: Ticket): Event[T] = Events.except(this, other)

  /**
   * Events conjunction
   */
  final def and[U, R](other: Event[U], merge: (T, U) => R)(implicit maybe: Ticket): Event[R] = Events.and(this, other, merge)

  /**
   * Event conjunction with a merge method creating a tuple of both event parameters
   */
  final def &&[U](other: Event[U])(implicit maybe: Ticket): Event[(T, U)] = Events.and(this, other, (p1: T, p2: U) => (p1, p2))

  /**
   * Transform the event parameter
   */
  final def map[U](mapping: T => U)(implicit maybe: Ticket): Event[U] = Events.map(this)(mapping)

  /**
   * Drop the event parameter; equivalent to map((_: Any) => ())
   */
  final def dropParam(implicit maybe: Ticket): Event[Unit] = Events.map(this)(_ => ())


  /** folds events with a given fold function to create a Signal */
  final def fold[A](init: A)(fold: (A, T) => A)(implicit maybe: Ticket): Signal[A] = Signals.fold(this, init)(fold)

  /** Iterates a value on the occurrence of the event. */
  final def iterate[A](init: A)(f: A => A)(implicit maybe: Ticket): Signal[A] = fold(init)((acc, _) => f(acc))
   ...
   */
  final def count()(implicit maybe: Ticket): Signal[Int] = fold(0)((acc, _) => acc + 1)

  /**
   * Calls f on each occurrence of event e, setting the Signal to the generated value.
   * The initial signal is obtained by f(init)
   */
  final def set[B >: T, A](init: B)(f: (B => A))(implicit maybe: Ticket): Signal[A] = fold(f(init))((_, v) => f(v))

  /** returns a signal holding the latest value of the event. */
  final def latest[S >: T](init: S)(implicit maybe: Ticket): Signal[S] = fold(init)((_, v) => v)

  /** Holds the latest value of an event as an Option, None before the first event occured */
  final def latestOption()(implicit maybe: Ticket): Signal[Option[T]] = fold(None: Option[T]) { (_, v) => Some(v) }

  /** calls factory on each occurrence of event e, resetting the Signal to a newly generated one */
  final def reset[S >: T, A](init: S)(factory: S => Signal[A])(implicit maybe: Ticket): Signal[A] = set(init)(factory).flatten()

  /**
   * Returns a signal which holds the last n events in a list. At the beginning the
   * list increases in size up to when n values are available
   */
  final def last(n: Int)(implicit maybe: Ticket): Signal[LinearSeq[T]] =
    fold(Queue[T]()) { (queue: Queue[T], v: T) =>
      if (queue.length >= n) queue.tail.enqueue(v) else queue.enqueue(v)
    }

  /** collects events resulting in a variable holding a list of all values. */
  final def list()(implicit maybe: Ticket): Signal[List[T]] = fold(List[T]())((acc, v) => v :: acc)

  /** Switch back and forth between two signals on occurrence of event e */
  final def toggle[A](a: Signal[A], b: Signal[A])(implicit maybe: Ticket): Signal[A] = maybe { implicit turn =>
  ...
  final def snapshot[A](s: Signal[A])(implicit maybe: Ticket): Signal[A] = maybe { turn =>
  ...
  final def switchOnce[A](original: Signal[A], newSignal: Signal[A])(implicit maybe: Ticket): Signal[A] = maybe { implicit turn =>
   ...
   */
  final def switchTo[S >: T](original: Signal[S])(implicit maybe: Ticket): Signal[S] = {
    val latest = latestOption
