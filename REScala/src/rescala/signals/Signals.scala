...
object Signals {
  ...
  def mapping[T](dependencies: Reactive*)(expr: Turn => T)(implicit maybe: MaybeTurn): Signal[T] = maybe { initialTurn =>
    makeStatic(dependencies.toSet, expr(initialTurn))((turn, _) => expr(turn))(initialTurn)
  }

  def lift[A1, A2, B](s1: Stateful[A1], s2: Stateful[A2])(expr: (A1, A2) => B)(implicit maybe: MaybeTurn): Signal[B] =
    mapping(s1, s2)(t => expr(s1.getValue(t), s2.getValue(t)))
  def lift[A1, A2, A3, B](s1: Stateful[A1], s2: Stateful[A2], s3: Stateful[A3])(expr: (A1, A2, A3) => B)(implicit maybe: MaybeTurn): Signal[B] =
    mapping(s1, s2)(t => expr(s1.getValue(t), s2.getValue(t), s3.getValue(t)))
