...
object Timer {
  ...
  def create(interval : Int) : Timer = new Timer(interval)
  ...
  def tickNext {    
    ...
    // register all timers to be triggered
    run foreach { globaltick -= _.timer.tick }
    
    // tick all
    globaltick(realSecs)
    
    // unregister all timers that were triggered
    run foreach { globaltick -= _.timer.tick }   
...
//  /** Returns a new Signal that counts the local time from now */
//  def localTime : Signal[Time] = {
//    val now = time.getValue
//    return SignalSynt {s: SignalSynt[Time] => time(s) - now}
//  }
//  
//  /** Returns a Signal which is true if the specified delay has passed */
//  def passed(delay : Time) : Signal[Boolean] = {
...
//    val delta = interval / 1000.0
//    val n = (window / delta).asInstanceOf[Int]
//    s.snapshot(tick).changed.last(n)
//  }
//  
//  // globally register this timer
//  Timer.register(this)
...
//    
//    // register all timers to be triggered
//    run foreach { x: Scheduled => {globaltick += (y=> x.timer.tick)  }}
//    
//    // tick all
//    globaltick(realSecs)
//    
