...
class DependenciesTestSuite extends AssertionsForJUnit with MockitoSugar {
  ...
  @Test def varsDoNotHaveDependentsByDefault(): Unit = {
    ...
    val v3 = Var(false)
    assert(v1.dependantCount() == 0)
    assert(v2.dependantCount() == 0)
    assert(v3.dependantCount() == 0)
  }
  ...
  @Test def signalsDoNotHaveDependenciesByDefault(): Unit = {
    ...
    val s3 = Signal { false }
    assert(s1.dependantCount() == 0)
    assert(s1.dependencyCount() == 0)
    assert(s2.dependantCount() == 0)
    assert(s2.dependencyCount() == 0)
    assert(s3.dependantCount() == 0)
  ...
  @Test def signalsCanHaveDependents(): Unit = {
    ...
    val s2 = Signal(10)
    val s3 = Signal { s1() + s2() }
    assert(s1.dependantCount() == 1)
    assert(s2.dependantCount() == 1)
    assert(s2.dependencyCount() == 0)
    assert(s3.dependantCount() == 0)
  ...
  @Test def fibonacciDependencies(): Unit = {
    ...
    val f3 = Signal { f1() + f2() }
    val f4 = Signal { f2() + f3() }
    assert(f1.dependantCount() == 1)
    assert(f2.dependantCount() == 2)
    assert(f3.dependantCount() == 1)
    assert(f3.dependencyCount() == 2)
    assert(f4.dependantCount() == 0)
