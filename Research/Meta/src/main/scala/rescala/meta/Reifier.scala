...
class EngineReifier[S <: Struct]()(implicit val engine: Engine[S]) extends Reifier[S] {
  ...
  def evaluateNecessaryReification(graph: DataFlowGraph): Unit = {
    @tailrec
    def findTriggeredObserve(log : List[MetaLog[_]]): Option[(DataFlowNode[_], DataFlowNode[_])] = log match {
    ...
    @tailrec
    def findTrigger(log : List[MetaLog[_]], observe: DataFlowNode[_]): Option[DataFlowNode[_]] = log match {
      case LoggedFire(DataFlowRef(trigger), _) :: tail  => if (graph.isDataFlowPossible(trigger, observe)) Some(trigger) else findTrigger(tail, observe)
      case LoggedSet(DataFlowRef(trigger), _) :: tail  => if (graph.isDataFlowPossible(trigger, observe)) Some(trigger) else findTrigger(tail, observe)
