...
class DataFlowGraph {
  ...
  protected[meta] def incomingDataFlow(node : DataFlowNode[_]): Set[DataFlowNode[_]] = node.dependencies.map(_refs.get).collect{ case Some(n) => n }
  protected[meta] def outgoingDataFlow(node : DataFlowNode[_]): Set[DataFlowNode[_]] = nodes.filter(_.dependencies.collect{ case DataFlowRef(n) => n }.contains(node))
  ...
  protected[meta] def removeLog(metaLog: MetaLog[_]): Unit = metaLog match {
    case LoggedFire(_, _) | LoggedSet(_, _) => _log.dequeueAll(_ == metaLog)
    case _ => throw new IllegalArgumentException("Cannot remove structural logs without causing graph inconsistencies!")
