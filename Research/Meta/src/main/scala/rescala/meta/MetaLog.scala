...
case class ObserverData[S <: Struct](onSuccess: Nothing => Unit, onFailure: Throwable => Unit, ticket: Ticket[S]) {
...
case class LoggedObserve[T, S <: Struct](override val node : DataFlowRef[T], observer : ObserverData[S]) extends MetaLog[T]