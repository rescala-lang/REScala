...
class MetaTest extends FunSuite {
  ...
  test("meta graph reification test") {
    ...
    assert(v.reify.now == 2, "variable setting to 2 not reified")
    assert(!fired, "observer added after reification prematurly triggered")
    v.reify.set(3)
    assert(v.reify.now == 3, "variable setting to 3 not reified")
  ...
  test("meta graph full dependency reification test") {
    ...
    assert(v.reify.now == 2, "variable setting to 2 not reified")
    assert(v2.reify.now == 3, "signal propagation to 3 not reified")
    assert(fired == 2, "event not fired or not both observers triggered")
    fired = 0
    e += ((x : Int) => { fired += 1 })
    api.set(v, 3)
    assert(v.reify.now == 3, "variable setting to 3 not reified")
    assert(v2.reify.now == 4, "signal propagation to 4 not reified")
