...
class SubsumableLockTransmittable {
  ...
  implicit def subsumableLockTransmittable[S](implicit messageTransmittable: Transmittable[M, S, M], serializable: Serializable[S]) = {
    new RemoteObjectTransmittable[SubsumableLock, SubsumableLock, Unit, Nothing, OperationRequest, OperationResponse, S] {
      ...
      override def instantiateMirror(value: Unit, request: (OperationRequest) => OperationResponse): SubsumableLock = new SubsumableLock {
        ...
        override def subsume(subsumableLock: SubsumableLock): SubsumableLock = request(SubsumeRequest(subsumableLock)).asInstanceOf[SubsumeResponse].res
        override def lock(): SubsumableLock = request(LockRequest).asInstanceOf[LockResponse].res
        override def getLockedRoot: Option[SubsumableLock] = request(GetLockedRootRequest).asInstanceOf[GetLockedRootResponse].res
      }
...
object SubsumableLockTransmittable extends SubsumableLockTransmittable {
  ...
  case class LockResponse(res: SubsumableLock) extends OperationResponse
  case class TryLockResponse(res: TryLockResult) extends OperationResponse
  case class GetLockedRootResponse(res: Option[SubsumableLock]) extends OperationResponse
  case class SubsumeResponse(res: SubsumableLock) extends OperationResponse
  ...
  case class SubsumeRequest(lock: SubsumableLock) extends OperationRequest {
