...
object ReactiveLocalClone {
  def apply[A](signal: Signal[A, FullMVStruct], host: FullMVEngine): Signal[A, FullMVStruct] = apply(signal)(CreationTicket.fromEngine(host))
  ...
  def apply[P](event: Event[P, FullMVStruct], host: FullMVEngine): Event[P, FullMVStruct] = apply(event)(CreationTicket.fromEngine(host))
