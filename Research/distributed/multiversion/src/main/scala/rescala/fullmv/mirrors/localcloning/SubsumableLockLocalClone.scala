...
class SubsumableLockLocalCloneProxy(fakeDelay: Duration, mirrorHost: SubsumableLockHost, localProxy: SubsumableLockProxy, reflectionHost: SubsumableLockHost) extends SubsumableLockProxy {
  override def getLockedRoot: Future[LockStateResult0] = FakeDelayer.requestReply(fakeDelay, localProxy.getLockedRoot)
  override def remoteTryLock(): Future[RemoteTryLockResult] = FakeDelayer.requestReply(fakeDelay, localProxy.remoteTryLock().map {
  ...
  override def remoteTrySubsume(lockedNewParent: SubsumableLock): Future[RemoteTrySubsumeResult] = FakeDelayer.requestReply(fakeDelay,  localProxy.remoteTrySubsume(SubsumableLockLocalClone(lockedNewParent, mirrorHost, fakeDelay)).map {
