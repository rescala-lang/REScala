...
class SubsumableLockLocalCloneProxy(fakeDelay: Duration, mirrorHost: SubsumableLockHost, localProxy: SubsumableLockProxy, reflectionHost: SubsumableLockHost) extends SubsumableLockProxy {
  override def getLockedRoot: Future[LockStateResult0] = FakeDelayer.requestReply(reflectionHost, mirrorHost, fakeDelay, localProxy.getLockedRoot)
  override def remoteTryLock(): Future[RemoteTryLockResult] = FakeDelayer.requestReply(reflectionHost, mirrorHost, fakeDelay, localProxy.remoteTryLock().map {
  ...
  override def remoteTrySubsume(lockedNewParent: SubsumableLock): Future[RemoteTrySubsumeResult] = FakeDelayer.requestReply(reflectionHost, mirrorHost, fakeDelay,  localProxy.remoteTrySubsume(SubsumableLockLocalClone(lockedNewParent, mirrorHost, fakeDelay)).map {
    case RemoteSubsumed => RemoteSubsumed
    case RemoteBlocked(newParent: SubsumableLock) => RemoteBlocked(SubsumableLockLocalClone(newParent, reflectionHost, fakeDelay))
    case RemoteGCd => RemoteGCd
  }(FullMVEngine.notWorthToMoveToTaskpool))
  override def remoteUnlock(): Unit = FakeDelayer.async(reflectionHost, mirrorHost, fakeDelay, localProxy.remoteUnlock())
  override def asyncRemoteRefDropped(): Unit = FakeDelayer.async(reflectionHost, mirrorHost, fakeDelay, localProxy.asyncRemoteRefDropped())
}
