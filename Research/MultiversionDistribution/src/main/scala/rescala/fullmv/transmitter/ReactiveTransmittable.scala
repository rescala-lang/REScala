...
//  /** [[FullMVTurnProxy]] **/
//  case class AsyncAddPhaseReplicator(turn: Host.GUID) extends Async[Nothing]
//  case class AsyncAddPredecessorReplicator(turn: Host.GUID) extends Async[Nothing]
...
//  /** [[FullMVTurnPhaseReflection]] **/
//  case class NewPhase(turn: Host.GUID, phase: TurnPhase.Type) extends Async[Nothing]
//  /** [[FullMVTurnPredecessorReflection]] **/
//  case class NewPredecessors(turn: Host.GUID, newPredecessors: CaseClassTransactionSpanningTreeNode[(Host.GUID, TurnPhase.Type)]) extends Async[Nothing]
//
...
//          host.getCachedOrReceiveRemote(predGuid, new FullMVTurnReflection(host, predGuid, predPhase, new FullMVTurnMirrorProxyToEndpoint(predGuid, endpoint))).instance
//        } else {
//          new FullMVTurnReflection(host, predGuid, predPhase, null)
//        }
//      }
//
//      val instance = host.getCachedOrReceiveRemote(guid, new FullMVTurnReflection(host, guid, phase, new FullMVTurnMirrorProxyToEndpoint(guid, endpoint))).instance match {
//        case reflection: FullMVTurnReflection =>
//          reflection.asyncNewPhase(phase)
//          reflection.asyncNewPredecessors(preds)
//          reflection
//        case notReflection =>
//          notReflection
//      }
