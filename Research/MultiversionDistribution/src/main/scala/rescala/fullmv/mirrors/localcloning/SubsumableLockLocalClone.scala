...
class SubsumableLockLocalCloneProxy(mirrorHost: SubsumableLockHost, localProxy: SubsumableLockProxy, reflectionHost: SubsumableLockHost) extends SubsumableLockProxy {
  ...
  override def remoteLock(): Future[SubsumableLock] = localProxy.remoteLock().map(SubsumableLockLocalClone(_, reflectionHost))(FullMVEngine.notWorthToMoveToTaskpool)
  ...
  override def remoteTrySubsume(lockedNewParent: SubsumableLock): Future[Option[SubsumableLock]] = localProxy.remoteTrySubsume(SubsumableLockLocalClone(lockedNewParent, mirrorHost)).map(_.map(SubsumableLockLocalClone(_, reflectionHost)))(FullMVEngine.notWorthToMoveToTaskpool)
