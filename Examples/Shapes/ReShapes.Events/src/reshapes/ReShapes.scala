...
object ReShapes extends SimpleSwingApplication {
  ...
  val drawingSpaceStateChanged = new ImperativeEvent[DrawingSpaceState] //#EVT
    ...
    contents += new Menu("Edit") {
      contents += new MenuItem(new UndoAction) {
        ...
        drawingSpaceStateChanged += { state => //#HDL
          ...
          currentState = state
          if (currentState != null)
            currentState.commandsChanged += updateCommands //#HDL
  ...
  reactions += {
    case SelectionChanged(ui.tabbedPane) =>
      drawingSpaceStateChanged(drawingSpaceState)
