...
object PayloadBlock {
  def createFrom(data: Array[Byte]): PayloadBlock = PayloadBlock(CanonicalBlock.PAYLOAD_BLOCK_TYPE_CODE, 0, BlockProcessingControlFlags(), 0, data)
  def createFrom(utf8_payload: String): PayloadBlock = PayloadBlock(CanonicalBlock.PAYLOAD_BLOCK_TYPE_CODE, 0, BlockProcessingControlFlags(), 0, Base64.getEncoder().encode(utf8_payload.getBytes(StandardCharsets.UTF_8)))
}
...
case class HopCountBlock(block_type_code: Int, block_number: Int, block_processing_control_flags: BlockProcessingControlFlags, crc_type: Int, data: Array[Byte]) extends CanonicalBlock {
