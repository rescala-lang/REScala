...
trait MetaEventPointer[+T] extends MetaReactivePointer[T] {
  def reify[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt]): Event[T, S] = reifier.reifyEvent(this)
...
trait MetaSignalPointer[+A] extends MetaReactivePointer[A] {
  def reify[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt]): Signal[A, S] = reifier.reifySignal(this)

  protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S]
...
case class ChangeEventPointer[+T](protected[meta] override var node : Option[ReactiveNode], base : MetaSignalPointer[T]) extends MetaEventPointer[Signals.Diff[T]] {
  override def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[Signals.Diff[T], S] = base.reify(reifier).change
}
case class ChangedEventPointer[+T](protected[meta] override var node : Option[ReactiveNode], base : MetaSignalPointer[T]) extends MetaEventPointer[T] {
  override protected [meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[T, S] = base.reify(reifier).changed
}
...
case class AndEventPointer[T, U, +R](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], other : MetaEventPointer[U], merger: (T, U) => R) extends MetaEventPointer[R] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[R, S] = base.reify(reifier).and(other.reify(reifier))(merger)
}
case class ZippedEventPointer[+T, +U](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], other : MetaEventPointer[U]) extends MetaEventPointer[(T, U)] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[(T, U), S] = base.reify(reifier).zip(other.reify(reifier))
}
case class MappedEventPointer[T, +U](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], mapping: (T) => U) extends MetaEventPointer[U] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[U, S] = base.reify(reifier).map(mapping)
}
case class FlatMappedEventPointer[T, +B](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], f: (T) => MetaEventPointer[B]) extends MetaEventPointer[B] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Event[B, S] = base.reify(reifier).flatMap(f(_).reify(reifier))
}
...
case class FoldedSignalPointer[T, A](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], init: A, fold: (A, T) => A) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).fold(init)(fold)
}
case class ToggledSignalPointer[+T, +A](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], a : MetaSignalPointer[A], b : MetaSignalPointer[A]) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).toggle(a.reify(reifier), b.reify(reifier))
}
case class SnapshotSignalPointer[+T, +A](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], s : MetaSignalPointer[A]) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).snapshot(s.reify(reifier))
}
case class SwitchOnceSignalPointer[+T, +A](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], original : MetaSignalPointer[A], newSignal : MetaSignalPointer[A]) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).switchOnce(original.reify(reifier), newSignal.reify(reifier))
}
case class SwitchToSignalPointer[+T <: A, +A](protected[meta] override var node : Option[ReactiveNode], base : MetaEventPointer[T], original : MetaSignalPointer[A]) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).switchTo(original.reify(reifier))
}
case class DelayedSignalPointer[+A](protected[meta] override var node : Option[ReactiveNode], base : MetaSignalPointer[A], n: Int) extends MetaSignalPointer[A] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[A, S] = base.reify(reifier).delay(n)
}
case class MappedSignalPointer[A, +U](protected[meta] override var node : Option[ReactiveNode], base : MetaSignalPointer[A],  mapping: (A) => U) extends MetaSignalPointer[U] {
  override protected[meta] def createReification[S <: Struct](reifier: Reifier[S, Signal, Event, Var, Evt])(implicit ticket: Ticket[S]): Signal[U, S] = base.reify(reifier).map(mapping)
}
