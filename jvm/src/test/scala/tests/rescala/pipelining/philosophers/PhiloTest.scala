...
import tests.rescala.pipelining.philosophers.PhilosopherTable.{ Thinking, Seating, Taken }
import scala.annotation.tailrec
import scala.util.Random
import rescala.graph.Committable
import tests.rescala.pipelining.philosophers.PhilosopherTable.Free
import tests.rescala.pipelining.philosophers.PhilosopherTable.Ready
...
class PhiloTest extends AssertionsForJUnit {
  ...
  def `eat!`(implicit engine: Engine[Turn]): Unit = {
    ...
    val threads = for (threadIndex <- Range(0, threadCount)) yield Spawn {
      while (!cancel) try {
        ...
        println(s"${seating.placeNumber} wants to eat")
        table.eatOnce(seating)
        println(s"${seating.placeNumber} ate once")
        table.engine.plan(seating.philosopher)(implicit t => {
          ...
          t.schedule(new Committable {
            ...
            override def commit(implicit t: Turn) = {
              println(s"THinking assertions for ${t}")
  ...
  @Test(timeout = 200000) def eatingContestsPipelining(): Unit = `eat!`(Engines.pipelining)
  @Test(timeout = 200000)
