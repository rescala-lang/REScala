...
object BundleCreation {
  ...
  def createBundleRdt(data: Array[Byte], dots: Dots, node: Endpoint, full_destination_uri: String, full_source_uri: String): Bundle = {
    val primary_block: PrimaryBlock = PrimaryBlock(
      version = 7,
      bundle_processing_control_flags = BundleProcessingControlFlags(do_not_fragment = true),
      crc_type = 0,
    ...
    val hop_count_block: HopCountBlock = HopCountBlock.createFrom(HopCount(hop_limit = 32, current_count = 0)).copy(block_number = 3)
