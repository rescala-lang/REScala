...
class MonntpyService(CoreService):
    """
    ...
    name: str = "MonntpyService"
    group: str = "moNNT.py"
    executables: Tuple[str, ...] = ()
    dependencies: Tuple[str, ...] = ("dtnd", )
    dirs: Tuple[str, ...] = ()
    configs: Tuple[str, ...] = ("start-monntpy.sh", )
    startup: Tuple[str, ...] = (
            "bash start-monntpy.sh",
        )
    validate: Tuple[str, ...] = ()
...
cur=$(pwd)
cd /app/moNNT.py
export SENDER_EMAIL={node.name}@monntpy.net
