...
class MembershipSuite extends munit.ScalaCheckSuite {
  ...
  property("Membership with simplepaxos")(MembershipSpec[simplified.Paxos, simplified.Paxos].property())
}

class MembershipSpec[C[_]: Consensus, D[_]: Consensus](using
    Bottom[C[Set[Uid]]],
    Bottom[D[Int]],
    Lattice[C[Set[Uid]]],
    Lattice[D[Int]],
  ...
  type LocalState = Membership[Int, C, D]
  ...
  override def genInitialState: Gen[State] =
    ...
    yield ids.map(id => (id, Membership.init[Int, C, D](ids.map(_.uid).toSet)))
...
//      )) :| s"updates only contain Uids of known members. Tried merging ${allUids(leftMembership.innerConsensus)} and ${allUids(rightMembership.innerConsensus)}" &&
  ...
  case class Upkeep(index: Int) extends Command:
    ...
    def newLocalState(states: Seq[(LocalUid, LocalState)]) =
      val (id, membership) = states(index)
      (id, membership.merge(membership.upkeep()(using id, LogHack(true))))
