...
object ResettableCounter {
  ...
  implicit class syntax[C](container: C) extends OpsSyntaxHelper[C, ResettableCounter](container) {
    ...
    /** Without using fresh, reset wins over concurrent increments/decrements
      ...
      */
    def fresh(using LocalUid)(): CausalMutator = {
    ...
    private def update(using LocalUid, IsCausalMutator)(u: (Int, Int)): C = {
    ...
    def increment(using LocalUid, IsCausalMutator)(): C = update((1, 0))

    def decrement(using LocalUid, IsCausalMutator)(): C = update((0, 1))
