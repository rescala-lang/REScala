...
class LensTest extends RETests {
    multiEngined { engine =>
      ...
      class testLens extends Lens[testType, Int] {
        ...
        def toModel(v: Int, m : testType): testType = testType(v - 3, m.stringValue)//Record("intValue" -> v, "stringValue" -> m.stringValue).asInstanceOf[testType]
      }

      test("Non-Bijective Lens ") {
        val a = LVar[testType](testType(5, "THISDOESNOTCHANGE")) //(Record("intValue" -> 5, "stringValue" -> "THISDOESNTCHANGE").asInstanceOf[testType])
