...
def constructIWAWithRequires[S <: Tuple, A](interaction: Expr[InteractionWithActs[S, A]],
...
def constructIWAWithEnsures[S <: Tuple, A](interaction: Expr[InteractionWithActs[S, A]],
                                                               expr: Expr[(S, A) => Boolean])
                                                              (using Quotes, Type[S], Type[A]): Expr[InteractionWithActs[S, A]] = '{
  val (inputs, fun, isStatic) =
    reactives.macros.MacroLegos.getDependencies[(S, A) => Boolean, ReSource.of[BundleState], reactives.core.StaticTicket[BundleState], true]($expr)
...
case class InteractionWithActs[S <: Tuple, A] private[dsl](private[dsl] val requires: Seq[Requires[S, A]] = Seq.empty,
