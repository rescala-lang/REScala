...
object DafnyGen {
  ...
  /** Generates Dafny code for the given LoRe Term node.
    ...
    * @return The generated Dafny code.
    */
  private def generate(node: Term, ctx: Map[String, NodeInfo]) = {
    node match
  ...
  /** Generates a Dafny type annotation for the given LoRe Type node.
    ...
    */
  private def generate(node: Type, ctx: Map[String, NodeInfo]): String = {
    node match
  ...
  /** Generates a Dafny Type annotation for the given LoRe SimpleType node.
    ...
    * @return The generated Dafny Type annotation.
    */
  private def generateFromSimpleType(node: SimpleType, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates a Dafny Type annotation for the given LoRe TupleType node.
    ...
    * @return The generated Dafny Type annotation.
    */
  private def generateFromTupleType(node: TupleType, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TArgT.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTArgT(node: TArgT, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TTuple.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTTuple(node: TTuple, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TIf.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTIf(node: TIf, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TSeq.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTSeq(node: TSeq, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TArrow.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTArrow(node: TArrow, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TAssert.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTAssert(node: TAssert, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TAssume.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTAssume(node: TAssume, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TReactive.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTReactive(node: TReactive, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TArith.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTArith(node: TArith, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TNum.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTNum(node: TNum, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TDiv.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTDiv(node: TDiv, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TMul.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTMul(node: TMul, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} * ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TAdd.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTAdd(node: TAdd, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} + ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TSub.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTSub(node: TSub, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} - ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TBoolean.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTBoolean(node: TBoolean, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TTrue.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTTrue(node: TTrue, ctx: Map[String, NodeInfo]): String = {
    "true"
  }
  ...
  /** Generates Dafny code for the given LoRe TFalse.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTFalse(node: TFalse, ctx: Map[String, NodeInfo]): String = {
    "false"
  }
  ...
  /** Generates Dafny code for the given LoRe TNeg.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTNeg(node: TNeg, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TLt.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTLt(node: TLt, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} < ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TGt.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTGt(node: TGt, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} > ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TLeq.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTLeq(node: TLeq, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} <= ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TGeq.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTGeq(node: TGeq, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} >= ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TEq.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTEq(node: TEq, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} == ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TIneq.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTIneq(node: TIneq, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} != ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TDisj.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTDisj(node: TDisj, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} || ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TConj.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTConj(node: TConj, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} && ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TImpl.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTImpl(node: TImpl, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TBImpl.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTBImpl(node: TBImpl, ctx: Map[String, NodeInfo]): String = {
    s"${generate(node.left, ctx)} <==> ${generate(node.right, ctx)}"
  }
  ...
  /** Generates Dafny code for the given LoRe TInSet.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTInSet(node: TInSet, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TQuantifier.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTQuantifier(node: TQuantifier, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TParens.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTParens(node: TParens, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TString.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTString(node: TString, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TFAcc.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTFAcc(node: TFAcc, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TFCall.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTFCall(node: TFCall, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TFunC.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTFunC(node: TFunC, ctx: Map[String, NodeInfo]): String = {
    // References:
  ...
  /** Generates Dafny code for the given LoRe TViperImport.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTViperImport(node: TViperImport, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TTypeAl.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTTypeAl(node: TTypeAl, ctx: Map[String, NodeInfo]): String = {
  ...
  /** Generates Dafny code for the given LoRe TFCurly.
    ...
    * @return The generated Dafny code.
    */
  private def generateFromTFCurly(node: TFCurly, ctx: Map[String, NodeInfo]): String = {
